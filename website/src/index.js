...
async function getRepoNameFetchAndDraw() {
  // get repo str (format: 'bytebase/star-history')
  let repo = "";
...
async function fetchDataAndDraw(repo, token) {
  ...
  try {
    ...
    if (location.hash === "") {
      ...
      // minimal sample of repo name 'a/b'
...
function drawAddTokenBtn(token) {
  ...
  document.getElementById("addTokenBtn").innerHTML =
    "<strong>" + verb + " access token</strong>";
  document.getElementById("modalCardTitle").innerHTML =
    verb + " GitHub access token";
}
